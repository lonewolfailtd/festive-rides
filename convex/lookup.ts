...
export const doi = action({
  ...
  handler: async (ctx, { doi }) => {
    ...
    const cleaned = doi.trim().replace(/^https?:\/\/(?:dx\.)?doi\.org\//i, "").replace(/^doi:\s*/i, "");
  ...
  handler: async (ctx, { url }) => {
    ...
    const authors = meta.author
      ? (() => {
          ...
          return [{ kind: "group" as const, name: meta.author! }];
        })()
      : [{ kind: "group" as const, name: meta.siteName ?? parsed.hostname }];
