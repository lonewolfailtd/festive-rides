      ...
      <header className="mb-8 flex items-center justify-between gap-4">
        <div>
          <p className="text-sm text-sky-400">
            <Link href="/uni" className="hover:text-sky-300">
              Uni tools
            </Link>{" "}
            / Analyser
          </p>
      ...
      {result && (
        <div className="space-y-6">
          ...
          <section className={sectionCard}>
            ...
            <blockquote className="mt-3 border-l-4 border-sky-500 bg-slate-950 px-4 py-3 text-sm italic text-slate-100">
              {result.keyQuestion}
            </blockquote>
          </section>
              ...
              <dl className="mt-3 space-y-3">
                {result.taskVerbs.map((t, i) => (
                  <div key={i} className="rounded-lg border border-slate-800 bg-slate-950 p-3">
                    ...
                    <dd className="mt-1 text-sm text-slate-200">{t.meaning}</dd>
                  </div>
                ))}
              </dl>
            )}
                ...
                {result.wordCountSplit.map((s, i) => {
                  ...
                  return (
                    <li key={i}>
                      ...
                      <p className="mt-1 text-xs text-slate-400">{s.purpose}</p>
                    </li>
                  );
                })}
              </ul>
          ...
          <section className={sectionCard}>
            ...
            {result.outline.length === 0 ? (
              <p className="mt-2 text-sm text-slate-400">No outline returned.</p>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        className="flex w-full items-center justify-between px-3 py-2 text-left text-sm font-medium text-slate-100 hover:bg-slate-900"
                      >
                        <span>{o.section}</span>
                        <span className="text-xs text-slate-400">
                          {open ? "Hide" : "Show"}
                      ...
                      {open && (
                        <ul className="list-disc space-y-1 px-8 pb-3 pt-1 text-sm text-slate-200">
                          {o.bullets.map((b, j) => (
                            <li key={j}>{b}</li>
                          ))}
                        </ul>
          ...
          <section className={sectionCard}>
            ...
            {result.researchKeywords.length === 0 ? (
              <p className="mt-2 text-sm text-slate-400">No keywords yet.</p>
            ) : (
              <div className="mt-3 flex flex-wrap gap-2">
